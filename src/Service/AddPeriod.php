...
class AddPeriod{
    ...
    public function test(Booking $booking){
        $nbjour = date_diff($booking->getStartDate(), $booking->getEndDate());
        $tests =  $manager->createQuery(
            'SELECT  c
            FROM App\Entity\Calendar c
            WHERE c.Date >= :startdate')
            ->setParameter('startdate',date_format($booking->getStartDate(), 'Y-m-d H:i:s'))
            ->setMaxResults((int)$nbjour->days)
            ->getResult();
